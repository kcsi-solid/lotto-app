 ...
 * 6을 곱하면 "그 번호가 이번 조합에 들어갈 확률"이 된다.
 * 45개 번호를 모두 똑같이 볼 때가 6/45 = 13.3%다.
 *
...
function basisHtml(unitPct, evenCombo) {
  ...
  return '<div class="basis">'
    ...
    + '<li><b>번호 확률</b> = softmax(점수 ÷ ' + t.toFixed(1) + ') × 6'
      + ' <span class="basis-sub">45개를 더하면 6.0. 45개 번호를 모두 똑같이 본다면 각각 '
      + unitPct.toFixed(1) + '%</span></li>'
    + '<li><b>조합 확률</b> = 6개 번호 확률을 모두 곱함'
      + ' <span class="basis-sub">45개를 모두 똑같이 본다면 (' + unitPct.toFixed(1) + '%)<sup>6</sup> = '
      + fmtComboProb(evenCombo) + '. 그보다 높으면 이 모델이 더 유망하게 본다는 뜻입니다.</span></li>'
    + '</ol>'
    + '<p class="basis-warn">이 확률은 <b>이 모델의 추정</b>입니다. '
    + '곱셈은 6번의 추출이 서로 독립이라고 가정한 값이라 엄밀한 결합확률도 아닙니다'
    + '(실제 로또는 비복원 추출). '
    + '<b>실제 1등 당첨 확률은 어떤 조합을 골라도 1/8,145,060 = 0.0000123%로 같습니다.</b></p>'
    + '</div>';
}
    ...
    chunk.forEach((g, i) => {
      ...
      if (showProb) {
        ...
        html += '<div class="game-foot">'
          + '<span class="combo-label">이 조합이 나올 확률</span>'
